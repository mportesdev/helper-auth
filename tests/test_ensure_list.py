...
@parametrize_with_cases("command", filter=~id_match_regex(".*_arg"))
def test_ensure_list_without_args(command):
    assert _ensure_list(*command) == ["helper"]


@parametrize_with_cases("command", filter=id_has_suffix("_one_arg"))
def test_ensure_list_with_one_arg(command):
    assert _ensure_list(*command) == ["helper", "--option"]


@parametrize_with_cases("command", filter=id_has_suffix("_more_args"))
